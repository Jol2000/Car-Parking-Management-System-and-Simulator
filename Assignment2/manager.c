...
// Global Variables
shared_memory_t shm;
queue_t levelQueue[LEVELS];
queue_t exitQueue[EXITS];
...
void *entranceLPR(void *arg){
    ...
    while (!fire){ 
        ...
        if (toDisplay != 'F' && toDisplay != 'X'){
            ...
            printf("NLPR - Time Spent Parked %0.2f\n", (double)parkTime/CLOCKS_PER_SEC);
...
void *statusDisplay() {
    ...
    for (;;) {
        ...
        for (int i =0; i < LEVELS; i++) {
            ...
            printf("Currently %d cars on level %d has a maximum capacity of %d\n", levelCapacity[i], i+1, CAPACITY);
        }
        printf("------------------------------------------------------------\n");         
        printf("Total Billing by Manager: $%.2f\n", generatedRevenue);//, totalRevenue);
        //printf("------------------------------------------------------------\n");
