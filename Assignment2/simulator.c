...
void *spawnCar();
void *simulateEntrance(void *arg);
void *simulateTempSensor(void *arg);

// Global Variables
char allowedPlates[100][7];
shared_memory_t shm;
queue_t entranceQueue[ENTRANCES];
...
int main()
{
    ...
    pthread_t simulateTempSensor_thread[LEVELS];
    ...
    for (i = 0; i < ENTRANCES; i++){
        ...
        *p = i;
        pthread_create(&entranceThread[i], NULL, &simulateEntrance, p);
    }

    for (i = 0; i < LEVELS; i++){
        int* z = malloc(sizeof(int));
        *z = i;
        pthread_create(&simulateTempSensor_thread[i], NULL, &simulateTempSensor, z);
    }
    ...
    for (i = 0; i < LEVELS; i++){
        pthread_join(simulateTempSensor_thread[i],NULL);
    }
}
...
void *spawnCar() {
    ...
    for (int i = 0;i < CAR_LIMIT;i++){
        ...
        printf("Plate %s arriving at entrance %d\n",plate,entranceCar + 1);
        // SPAWN CAR THREAD 
...
// Generate Random Numbers in Upper and Lower Range
...
// Print plates from supplied textfile
void printFile(){
	printf("\n Plates: \n");    
    for(int i = 0; i < 100; i++)
    {
...
// Generates Plate (Random/Allowed)
char* generatePlate(int probability){
