...
{ 
    ...
    while(lim < LOOPLIM) {
        ...
        for (int i = 0; i < LEVELS; i++)
        {
            ...
            printf("Temperature sensor reading from level %d = %d°C\n", i+1, rawData[i][j% ARSIZE]);
            if (j > 4) {
...
// Array Sorting
// Reference: https://www.includehelp.com/c-programs/calculate-median-of-an-array.aspx
void arraySort(int16_t array[ARSIZE] , int n)
{ 
    // Declare local variables
...
// Function checks if the upper bound of a loop exceeds or matches limit
// and terminates the program with an error message if true
void loopLim(int i)
{
	if (i >= LOOPLIM)
	{
		printf("Error: Upper Bound Loop Limit Exceeded. Program Exiting...");
		exit(1);
	}
}
